"""
Rúbrica | Semana 10 | Jesús Domínguez

1. Diseñe una solución para indicar el vértice con mayor grado de salida en un grafo
(si hay más de uno con el mismo valor, indicarlos todos).

2. Diseñe una solución para indicar si existe un camino dual entre dos vértices, es decir, 
si existe un camino del vértice inicial al vértice final, y al mismo tiempo, 
existe un camino del vértice final al vértice inicial en un grafo dirigido no valorado. 
Los vértices son indicados por el usuario.
"""
import sys
...
class ListaAdyacencia:
    ...
    # 1. Indicar el vértice con mayor grado de salida en un grafo
    def mayorGradoSalida(self):
        grados = []
        for vertice in self.verVertices():
            gradoTemporal = self.gradoSalidaVertice(vertice)
            grados.append(gradoTemporal)
        
        mayor = max(grados)
        repeticionesMayor = grados.count(mayor)
        
        # Cuando SOLO hay un mayor
        if repeticionesMayor == 1:
            indiceMayor = grados.index(mayor) 
            return self.verVertices()[indiceMayor], grados[indiceMayor]
        
        # Cuando hay VARIOS mayores
        pos = 0
        listaMayores = []
        for grado in grados:
            if grado == mayor:
                listaMayores.append((self.verVertices()[pos], grado))
            pos = pos + 1

        return listaMayores
    
    # 2. Camino dual entre dos vértices (grafo no varolado)
    def caminoDualVertices(self, verticeInicial, verticeFinal):
        caminoIda = self.verCaminosDosVertices(verticeInicial, verticeFinal)
        caminoVuelta = self.verCaminosDosVertices(verticeFinal, verticeInicial)

        if caminoIda is None or caminoVuelta is None:
            return None

        if len(caminoIda) == 0 or len(caminoVuelta) == 0:
            return False
...
lista_01.adicionarArco("B", "E")
lista_01.adicionarArco("C", "D")
lista_01.adicionarArco("D", "A")
lista_01.adicionarArco("E", "D")
lista_01.adicionarArco("E", "C")
lista_01.adicionarArco("D", "F")

print(f"Grafo 01: {lista_01}")

print(f"Vértice con mayor grado salida: {lista_01.mayorGradoSalida()}")

print(f"Camino dual entre 'A' y 'E': {lista_01.caminoDualVertices('A', 'E')}") # True
print(f"Camino dual entre 'A' y 'D': {lista_01.caminoDualVertices('A', 'D')}") # True
print(f"Camino dual entre 'C' y 'F': {lista_01.caminoDualVertices('C', 'F')}") # False
print(f"Camino dual entre 'E' y 'F': {lista_01.caminoDualVertices('E', 'F')}") # False
